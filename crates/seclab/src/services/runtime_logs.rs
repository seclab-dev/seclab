...
/// 返回运行日志根目录。开发环境可通过 `SECLAB_LOG_DIR` 覆盖。
...
pub fn service_log_dir(service: &str) -> ApiResult<PathBuf> {
    match service {
        "seclab" | "agent" => Ok(runtime_log_root().join(service)),
...
/// 查询本机 seclab/agent 运行日志文件列表。
pub async fn list_runtime_log_files(
    service_filter: Option<&str>,
) -> ApiResult<Vec<RuntimeLogFile>> {
    let mut files = Vec::new();
    let services: Vec<&str> = match service_filter {
        Some("seclab") => vec!["seclab"],
...
fn local_node_name(service: &str) -> Option<String> {
    match service {
        "seclab" => Some("SecLab".to_string()),
        "agent" => Some("Local Agent".to_string()),
        _ => None,
    }
}
...
mod tests {
    ...
    fn test_query(keyword: Option<&str>) -> RuntimeLogQuery {
        RuntimeLogQuery {
            service: "seclab".to_string(),
            node_id: None,
            file_name: "seclab.log".to_string(),
            level: None,
            target: None,
    ...
    fn parse_runtime_log_line_extracts_fields_source() {
        let raw = r#"{"timestamp":"2026-06-18T12:00:00+08:00","level":"INFO","target":"seclab::api","fields":{"message":"request completed","username":"admin","client_ip":"127.0.0.1","hidden":"needle"}}"#;
    ...
    fn parse_runtime_log_line_extracts_top_level_client_ip_source() {
        let raw = r#"{"timestamp":"2026-06-18T12:00:00+08:00","level":"INFO","target":"seclab::node","message":"node updated","client_ip":"10.0.0.12"}"#;
    ...
    fn parse_runtime_log_line_does_not_use_node_name_as_source() {
        let raw = r#"{"timestamp":"2026-06-18T12:00:00+08:00","level":"INFO","target":"seclab::node","message":"node updated","nodeName":"edge-01"}"#;
