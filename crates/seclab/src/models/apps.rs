...
/// 按排序读取应用目录列表。
pub async fn list_apps(pool: &DbPool, locale: Option<&str>) -> sqlx::Result<Vec<AppSummary>> {
    ...
    let suite_records = sqlx::query_as::<_, SuiteAppRecord>(
        r#"
        SELECT e.app_id,
               e.suite_instance_id,
               e.app_entry_id,
...
mod tests {
    ...
    async fn list_apps_returns_seeded_rows() {
        ...
        let apps = list_apps(&pool, None).await.unwrap();
