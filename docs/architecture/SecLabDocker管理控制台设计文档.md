...
全局操作日志按事件来源展示主控、本地节点或具体子节点，并可使用来源节点筛选。远程 Agent 在实际操作终态先写入本地 durable outbox，再由活动机器会话幂等上报；Master 不为同一次 Agent Docker 操作重复生成审计事件。

## 子节点 Compose 镜像策略

子节点创建 Compose 项目或在未勾选“部署前拉取最新镜像”时重新部署，会先由 Agent 使用 `docker compose config --images` 解析变量展开后的最终镜像引用。每个镜像依次复用子节点缓存、尝试从主控传输，并在主控没有镜像、传输失败或运行时暂不可用时回退到子节点配置的镜像仓库。镜像准备完成后，Compose 使用禁止隐式拉取的参数启动，避免绕过上述策略访问 Docker Hub。

勾选“部署前拉取最新镜像”时，由子节点直接从镜像仓库刷新并部署，不读取或更新主控上的同名镜像。本地节点维持 Docker Compose 自身的拉取行为。
