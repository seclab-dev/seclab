...
const mailCapabilityRows = computed(() =>
  ((props.config?.capabilities as string[] | undefined) || []).map((capability) => ({
    capability,
  })),
)
...
const mailMessageRows = computed(() => {
  if (props.protocol === 'imap') {
    const mailboxes =
      (props.config?.mailboxes as Record<string, Array<Record<string, unknown>>> | undefined) || {}
  ...
  return (props.config?.messages as Array<Record<string, unknown>> | undefined) || []
})

const mailCommandRows = computed(
  () => (props.config?.custom_responses as Array<Record<string, unknown>> | undefined) || [],
)
</script>
  ...
  <div class="section-card">
    ...
    <div class="table-scroll-container">
      ...
      >
        ...
        <template #event="{ row }">
          <SecLabTag :type="row.event_type === 'exploit_attempt' ? 'danger' : 'info'" size="small">
